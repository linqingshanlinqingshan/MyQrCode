...
package com.example.administrator.myqrcode.qrcode.zxing.view;
...
//        Bitmap laserLineBitmap = null;

            //图片资源文件转为 Bitmap
//        if (laserLineBitmap == null) {
//
//            laserLineBitmap = BitmapFactory.decodeResource(getResources(), R.mipmap.laser);
//        }

//        LogUtil.logDebug("----->画扫描线", " left = " + frame.left + " top = " + frame.top + " right = " + frame.right + " bottom = " + frame.bottom);
//        ToastUtil.showToastSHORT("----->画扫描线" + " left = " + frame.left + " top = " + frame.top + " right = " + frame.right + " bottom = " + frame.bottom);

            //线条图片
//            Rect laserRect = new Rect(frame.left, frame.top + ((frame.bottom - frame.top) / 2) + i - 6, frame.right
//                    , frame.top + ((frame.bottom - frame.top) / 2) + ScreenUtil.dip2px(getContext(), 1) + i + 6);

            Rect laserRect = new Rect(frame.left - 1, frame.top + i - 1, frame.right + 1, frame.top + 1 + i);


//        canvas.drawBitmap(laserLineBitmap, null, laserRect, null);


            Drawable drawable = getResources().getDrawable(R.mipmap.laser);

            drawable.setBounds(laserRect);
